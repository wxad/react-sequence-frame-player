...
const ArtDots = () => {
  ...
  const { mount, unmount, createCanvas, background, noFill, stroke, noise, noiseSeed, resizeCanvas, cos, sin, TWO_PI } =
    p5i();
  ...
  const draw = ({ circle }: { circle: (x: number, y: number, d: number) => void }) => {
    background('#fafafa');
  ...
  return <div ref={elRef} className="fixed inset-0 pointer-events-none" />;
};
